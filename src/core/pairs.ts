 ...
 * and the mapping has no enumerator — the only way to learn the list is to
 ...
 * with the mapping it is reconstructing is wrong by definition — one multicall
 * settles it.
 */
...
export interface PairToken {
   ...
   * factory itself rejects at launch — surfacing it here explains the refusal
...
export interface ReadPairsOptions {
   ...
   *
   * `pairTokenEconomics(address(0))` returns zeros — the native asset is not
