 ...
 * balance, meaning the value the transaction carried, then TAKE the token to the
 ...
 *
 * The quoter is `nonpayable`, swapping inside a call that reverts with the
 * result, so this is an `eth_call` rather than a read. It is the only firm number
