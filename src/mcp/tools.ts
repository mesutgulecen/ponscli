 ...
 * approvals and the ordering around them — handing an agent a second piece of
...
export function registerTools(server: McpServer, context: McpContext): void {
  ...
  server.registerTool(
    'pons_endpoints',
    {
      ...
      description:
        'Probe every configured endpoint with the calls the CLI actually makes — a contract read, a log query, a historical state read — rather than a liveness ping.',
      annotations: { readOnlyHint: true, openWorldHint: true },
    },
    async () => {
