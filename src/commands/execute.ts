 ...
 *
 * Four modes share it — print, `--unsigned`, `--dry-run`, `--confirm` — and
...
function renderTransaction(transaction: NonNullable<ExecutePayload['transaction']>, paint: Painter): string[] {
  ...
  if (transaction.status === 'unconfirmed') {
    lines.push(
      paint('grey', `  broadcast to ${transaction.endpoint} but no receipt yet — it may still land`),
...
export interface RunPlanOptions {
   ...
   * floor forward — a freshly computed floor falls with the price, so a trade
