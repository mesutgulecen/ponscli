...
Every write builds a **Plan** first — what it will send, to whom, what it expects back, and what is worth warning about — and the same Plan is what gets simulated and what gets signed, so the three cannot drift apart.
...
A launch is the one thing here that cannot be undone. The command prints the whole of what you are committing to — the address the token will land at, the supply, where it graduates, the fee split, what the snipe tax will do — before it builds a transaction, and it checks locally everything the factory would reject on chain.
...
`pons pairs` lists the assets a launch may be priced in — native ETH plus the twenty-three approved tokens: tokenised equities, two index funds and one stablecoin — in the order [the Pons create page](https://www.ponsfamily.com/launchpad/create) renders them, so the two agree row for row.
...
| `rpc.alchemyKey` | `PONS_ALCHEMY_KEY` | unset — leaving it unset disables the paid tier |
...
The full document — every decision, and the measurement behind it — is [`docs/architecture/ponscli.md`](docs/architecture/ponscli.md). The shape in brief:
...
A simulation proves one call against one block. It cannot prove that an approval lands and the sell in the *next* block then succeeds, or that a launch confirms and the curve it created is buyable a block later — and those are the failures that reach a user.
...
Every sequence carries a control that must fail — without it, a state override could be doing the work that the sequence appears to prove.
