...
This document is the single reference for `ponscli`: architectural decisions, measured protocol facts and command surface. Every claim about how Pons behaves records the measurement behind it; if a fresh measurement contradicts one, the document is what is wrong.
...
- **The MCP server signs nothing.** It loads no keystore, prompts for no
  password and broadcasts nothing; a test asserts that no tool is named for
  sending and that every one carries `readOnlyHint`. A CLI is driven by
  somebody who typed the command, while an MCP server is driven by a model a
  prompt can steer, so the worst a confused agent should manage here is a bad
  quote. [`docs/agents.md`](../agents.md) is the reference for callers.
