...
Finish a V2 curve that has raised its threshold. **Anyone may call this** — a
...
read, a log query, a historical state read — never a liveness ping. One public
...
| `watch` (V2) | the launch's curve, `PonsV2LaunchFactory` | `eth_getLogs` — `CurveBuy`, `CurveSell`, `BuybackLocked`, `LaunchSwept`, `PoolGraduated` |
| `watch` (V1) | the Uniswap V3 pool, `PonsLaunchLocker` | `eth_getLogs` — `Swap`, `Mint`, `Burn`, `FeesClaimed` |
| `tx` | — | `eth_getTransactionByHash`, `eth_getTransactionReceipt`, `eth_call` replay |
...
| `wallet transfer` / `sweep` | — | native transfer, or ERC-20 `transfer` |
| `doctor` | — | `eth_chainId`, `eth_blockNumber`, `eth_call`, `eth_getLogs`, archive read |
| `config` | — | local filesystem only |

Addresses are in [`src/chain/addresses.ts`](../src/chain/addresses.ts). Protocol
*parameters* — fees, tax windows, the approved pair list — are never hard-coded;
