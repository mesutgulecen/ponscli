...
| `pons_pairs` | none | Approved quote assets, in approval order. |
...
| `pons_endpoints` | none | RPC endpoint health, probed with real calls. |
...
Amounts *accepted* as input are human-scaled, such as `0.05`, `50%` or `all`,
because that is what a person says. The asymmetry is deliberate.
...
back, and what is worth warning about. That same Plan is what gets simulated and
